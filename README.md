...
### 星球粒子系统 (WebARPlanetParticles.html) ⭐ **全新**
- **橙色星球核心**：8,000个橙色粒子构成绚烂星球
- **白色星环**：4,000个白色粒子环绕星球
- **粒子特性**：
  - 大尺寸高亮度（星球4.0，星环2.5）
  ...
  - 球面表面显现效果
- **手势控制**：
  - **握拳缩小**：相机快速靠近星球（速度先快后慢）
  - **张手放大**：相机快速远离星球（速度先慢后快）
  - **挥手旋转**：手部移动同步旋转星球
  - **智能距离控制**：8-30距离范围，平滑过渡
- **视觉特效**：
  - 球面脉动动画（可开关）
  - 自动旋转模式（可开关）
  - 星环发光控制（可开关）
...
├── WebARParticleSystem.html    # 粒子互动系统（16,000粒子）
├── WebARPlanetParticles.html   # 星球粒子系统（12,000粒子）⭐全新
├── WebARChristmasTree.html      # 圣诞树互动系统（新增C键快捷键）
├── README.md                    # 项目文档
└── screenshots/                 # 截图（可选）
```
